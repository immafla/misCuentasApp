...
export const NewSale = ({ open, setOpen }: IActionsModal) => {
  const [pasted, setPasted] = useState<any>();
  ...
  useEffect(() => {
    const handleKeyDown = (event: any) => {
      ...
      if (event.key === 'Enter') {
        if (bufferRef.current.length > 2) { // Evitar falsos positivos
          console.log("✅ Código detectado:", bufferRef.current);
          setPasted(bufferRef.current);
          setListSelectedProducts((prevState) => [
            ...prevState, {
              name: bufferRef.current,
              price: "$5000",
            }
          ]);
          // AQUÍ LÓGICA DE TU APP (ej: buscar producto)
